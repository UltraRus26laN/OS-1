...
{
    int start; int size;
    int count; int chunk_addr;
    struct cache* next;
} cache;

int max_cache = 300; void* cache_memory = NULL;

typedef struct memory_manager
{
    void *memory; int allocated_memory;
    int max_allocated; chunk *first_chunk;
    chunk *last_chunk; cache *_cache;
    int DEALLOCATED;
} memory_manager;
...
bool find_free_space(cache** cur, int need_space)
{
    ...
    for (cache* temp = my_manager._cache; temp != NULL; temp = temp->next)
    {
        if (temp->next != NULL)
        {
            ...
            if(need_space < free_s) { *cur = temp; return true; }
        }
        else { if(need_space < (max_cache - (temp->start + temp->start))) { *cur = temp; return true; } }
    }
    return false;
}
...
cache* find_cache(int start_addr)
{
    for (cache* temp = my_manager._cache; temp != NULL; temp = temp->next)
    {
        if (temp->chunk_addr == start_addr){ temp->count++; return temp; }
    }
    return NULL;
}
...
{
    int max_count = -1; cache* prev = NULL;
    if (my_manager._cache != NULL){ max_count = my_manager._cache->count;} else return false;
    for (cache* temp = my_manager._cache; temp != NULL; temp = temp->next)
    {
        if (temp->next != NULL && temp->next->count > max_count)
        {
            prev = temp; max_count = temp->next->next->count;
        }
    }
    if (prev)
    {
...
{
    cache* prev = NULL; if (my_manager._cache == NULL) return;
...
void write_cache(chunk* to_write_chunk)
{
    ...
    if (e)return; if (to_write_chunk->size >= max_cache){ return; }
    cache* write_to = NULL;
...
segment_addr m_malloc(int size_of_chunk, m_err_code *error)
{
    ...
    {
        *error = M_ERR_ALLOCATION_OUT_OF_MEMORY; return &my_manager.max_allocated + 1;
    }
    chunk *temp = malloc(sizeof(chunk)); temp->next = NULL;
    ...
    *error = M_ERR_OK; return &temp->start_pointer;
}
...
m_err_code m_read(segment_addr read_from_id, void *read_to_buffer, int size_to_read)
{
    ...
    if(error == M_ERR_OK)return M_ERR_OK; chunk* temp = NULL;
    for(chunk* i = my_manager.first_chunk; i != NULL; i = i->next)
    {
        if(i->start_pointer == *read_from_id)
        {
            temp = i; write_cache(temp);
        }
    }
    if(temp == NULL)return M_ERR_INVALID_CHUNK; if(size_to_read > temp->size)return M_ERR_OUT_OF_BOUNDS;
...
m_err_code m_write(segment_addr write_to_id, void *write_from_buffer, int size_to_write)
{
    ...
    for(chunk* i = my_manager.first_chunk; i != NULL; i = i->next)
    {
        if(i->start_pointer == *write_to_id){ temp = i; }
    }
    if(temp == NULL)return M_ERR_INVALID_CHUNK; if(temp->size < size_to_write)return M_ERR_OUT_OF_BOUNDS;
    over_write_cache(*write_to_id, temp); memcpy(my_manager.memory + temp->start_pointer, write_from_buffer, size_to_write);
    return M_ERR_OK;
}
...
void m_init(int size)
{
    ...
    my_manager.DEALLOCATED = -1; my_manager.memory = malloc(size);
    cache_memory = malloc(max_cache); my_manager.allocated_memory = 0;
    my_manager.max_allocated = size; my_manager.first_chunk = NULL;
    my_manager.last_chunk = NULL;
}
